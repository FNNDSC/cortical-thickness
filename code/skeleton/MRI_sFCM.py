...
class MRI_sFCM():
    ...
    def run(self):
        ...
        for i in range(len(intensities)):
            if prob[i][0] > self.args.THRESHOLD:
                self.outputDataL1[_1DTo3D[i]] = intensities[i]
                self.outputMaskL1[_1DTo3D[i]] = 1
            elif prob[i][1] > self.args.THRESHOLD:
                self.outputDataL2[_1DTo3D[i]] = intensities[i]
                self.outputMaskL2[_1DTo3D[i]] = 1
    ...
    def plot(self, x, data, name="sFCM"):
        data = data.T
    ...
    def argumentParser(self):
        ...
        parser.add_argument("-inVOL", "--IN_VOL",action="store",dest="IN_VOL",type=str, default="sFCM_input.nii", help="input binarize .nii file containing CSF & GM")
        parser.add_argument("-outPath", "--OUT_PATH",action="store",dest="OUT_PATH",type=str, default="/neuro/labs/grantlab/research/MRI_processing/jose.cisneros/CorticalThickness/Results/FCB028/temp", help="output folder")
        parser.add_argument("-outCSF", "--OUT_CSF",action="store",dest="OUT_CSF",type=str, default="sFCM_csf.nii", help="output binarize .nii file containing improved CSF")
